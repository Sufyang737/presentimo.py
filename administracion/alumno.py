...
class Alumno:
    ...
    def save(self, nombre, apellido1, apellido2, turno, curso, fecha, presentismo):
        try:
    ...
    def consultar_alumnos(self):
        try:
            cursor = self.conexion.cursor()
            cursor.execute("SELECT * FROM alumnos a inner join asistencia asi on a.id=asi.idAlumno")
    ...
    def buscar_por_curso(self, curso):
        try:
            cursor = self.conexion.cursor()
            cursor.execute(f"SELECT * FROM alumnos a inner join asistencia asi on a.id=asi.idAlumno WHERE a.curso = '{curso}'")
            data = cursor.fetchall()
            cursor.close()
            return data
        except:
    ...
    def editar_datos(self, nombre, apellido1, apellido2, turno, curso, fecha, presentismo, id):
        try:
            cursor = self.conexion.cursor()
